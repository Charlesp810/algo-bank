...
/*minHeightBst (with insert method) Time: O(nlog(n)) Space: O(n) */
function minHeightBst(array) {
  return constructMinBst(array, null, 0, array.length - 1)
}

function constructMinBst(array, bst, startIdx, endIdx) {
  if (endIdx < startIdx) return
  let midIdx = Math.floor((startIdx + endIdx) / 2)
  let addValue = array[midIdx]
  if (bst === null) {
    bst = new BST(addValue)
  } else {
    bst.insert(addValue)
  }
  constructMinBst(array, bst, startIdx, midIdx - 1)
  constructMinBst(array, bst, midIdx + 1, endIdx)
  return bst
}
...
/*minHeightBst (WITHOUT insert method) Time: O(n) Space: O(n) */
function minHeightBst(array) {
  return constructMinBst(array, 0, array.length - 1)
}

function constructMinBst(array, startIdx, endIdx) {
  if (endIdx < startIdx) return null;
  let midIdx = Math.floor((startIdx + endIdx) / 2);
  const bst = new BST(array[midIdx]);
  bst.left = constructMinBst(array, startIdx, midIdx - 1)
  bst.right = constructMinBst(array, midIdx + 1, endIdx)
  return bst
}
